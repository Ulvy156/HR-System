<template>
  <section class="section-dashboard dark:bg-blue-400 flex flex-col gap-y-3">
    <!-- navbar -->
    <CommonCollapse :closeCollapse="item.isCollapseActive" :collapseTitle="item.name + item.router"
      v-for="item in routerList" :key="item.name" @click="getPreviousActive(item.name + item.router)">
      <template #title>
      ...
      <template v-if="item.ul.length > 0" #body>
        <ul class="text-black bg-none p-3 px-5 flex flex-col gap-y-4">
          <li @click="getPreviousIdLI(subItem.name)" v-for="subItem in item.ul" :key="subItem.name" :id="subItem.name"
