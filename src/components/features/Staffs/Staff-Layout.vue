        ...
        @click="getPreviousActive(item.name+item.router)" >
          ...
          <template v-if="item.ul.length > 0" #body>
            <ul class="text-black bg-none p-3 px-5 flex flex-col gap-y-4">
              <li @click="getPreviousIdLI(subItem.name)"
              v-for="subItem in item.ul" :key="subItem.name"
              :id="subItem.name"  class="flex items-center gap-x-5 ">
...
// Icons
const IconClipboard = markRaw(defineAsyncComponent(() => import('@/components/icons/IconClipboard.vue')));
const IconLeave = markRaw(defineAsyncComponent(() => import('@/components/icons/IconLeave.vue')));
const IconCandidate = markRaw(defineAsyncComponent(() => import("@/components/icons/IconCandidate.vue")))
